...
function VectorAdd() {
  ...
  var D=new Uint32Array(BUFFER_SIZE);
...
var program2 = cl.linkProgram(context, [device], null, [helper, main]);
  ...
  var dBuffer = cl.createBuffer(context, cl.MEM_READ_ONLY, size);

  // Create buffer for C to read results
  var cBuffer = cl.createBuffer(context, cl.MEM_WRITE_ONLY, size);
  var eBuffer = cl.createBuffer(context, cl.MEM_WRITE_ONLY, size);
  ...
  try {
    ...
    kernel2= cl.createKernel(program2, "vadd");
    argsKernel= cl.createKernel(program, "putv");
    argsKernel2= cl.createKernel(program2, "putv");
  }
  catch(err) {
  ...
  // Set kernel args2
  cl.setKernelArg(argsKernel2, 0, "int*", dBuffer);
  cl.setKernelArg(argsKernel2, 1, "int*", bBuffer);
  cl.setKernelArg(argsKernel2, 2, "int*", eBuffer);
  cl.setKernelArg(argsKernel2, 3, "uint", BUFFER_SIZE);
  ...
  cl.enqueueWriteBuffer (queue, dBuffer, true, 0, D.length*Uint32Array.BYTES_PER_ELEMENT, D);
  ...
  console.info("using enqueueNDRangeKernel1");
    cl.enqueueNDRangeKernel(queue, argsKernel, 1,
      ...
      [1],
      null);
    cl.enqueueNDRangeKernel(queue, argsKernel2, 1,
      null,
      [1],
      null);
  ...
  cl.enqueueNDRangeKernel(queue, kernel, 1,
      ...
      [BUFFER_SIZE],
      null);
    cl.enqueueNDRangeKernel(queue, kernel2, 1,
      null,
      [BUFFER_SIZE],
      null);
console.log('MSEC2', Date.now()-t2);
  ...
  var E=new Uint32Array(BUFFER_SIZE);
  cl.enqueueReadBuffer (queue, cBuffer, true, 0, C.length*Uint32Array.BYTES_PER_ELEMENT, C);
  cl.enqueueReadBuffer (queue, eBuffer, true, 0, E.length*Uint32Array.BYTES_PER_ELEMENT, E);
